...
def factors():
    ...
    print("<factors_sat> 'sat_channel_9' - Water vapor satelite data")
    print("<factors_sat> 'sat_channel_14' - Infrared satelite data")
    print("<factors_sat> 'sat_truecolor' - Simulated true-color satelite data")
...
def Retrieve(Time, factors, values):
    ...
    if "sat_channel_2" in factors:
        sat_dat_2 = FetchData('east', 2, Time,'CONUS').dat
    ...
    if "sat_channel_9" in factors:
        sat_dat_9 = FetchData('east', 9, Time,'CONUS').dat
        sat_img = declarative.ImagePlot()
    ...
    if "sat_channel_14" in factors:
        sat_dat_14 = FetchData('east', 14, Time,'CONUS').dat
        sat_img = declarative.ImagePlot()
    ...
    if "sat_truecolor" in factors:
        sat_trc = MakeTruecolorSat(FetchData('east', 1, Time,'CONUS'), FetchData('east', 2, Time,'CONUS'), FetchData('east', 3, Time,'CONUS'))
